...
protocols are described as in Figures \ref{fig:pak} and \ref{fig:ppk}. \comment{Yi: Any idea what's going on here?}
...
As for the security of the protocols, Boyko, MacKenzie and Patel has developed a new formal model for PAKEs in~\cite{BoMaPa00}, with which they have proved that PAK is secure in the random oracle model and PPK is secure in the implicit-authentication model, provided DDH is intractable. The newly proposed model and definition of security does not correspond directly to the four properties mentioned earlier in this section. However, it can be extrapolated from the proof that under the appropriate model and assumptions, the protocols are indeed able to provide forward secrecy as well as secure against online and offline dictionary attacks.
