...
\frame{  
  ...
  Instead, modern PAKEs use \emph{zero-knowledge proof}, \emph{exponentiation}, or \emph{hash} of expression w/ password
}
...
\frame{  
 ...
 \item[] Bob picks \hspace{0.015in} $x_3 \in_R [0,q-1]$ and $x_4 \in_R [1,q-1]$
 \item[]\pause
...
\frame{  
  ...
  Alternative PAKE, via hashing password with random elements and powering \\
   \vspace{0.3in}
...
\begin{frame}{Dragonfly (Harkins 2012)}
   Alternative PAKE, using discrete log / CDH assumption \\
