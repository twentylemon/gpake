...
\frame{  
  ...
  Work in small subgroups $\rightarrow$ more efficient than D-H, etc.
  \vspace{0.3in}
  ...
  \scalebox{0.7}{
    \begin{tikzpicture}
        \matrix (m)[matrix of nodes, column sep=2cm, row sep=8mm, nodes={draw=none, anchor=center, text depth=0pt},ampersand replacement=\&]{
            Alice                                       \&                   \& Bob                                       \\
            randomly choose $x_a \in \mathbb{Z}_p^*$    \& $[\alpha^{x_a}]_\pi$\&                                           \\
                                                        \& $[\alpha^{x_b}]_\pi$\& randomly choose $x_b \in \mathbb{Z}_p^*$  \\
        };
